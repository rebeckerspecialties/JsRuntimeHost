        ...
        if (env.Global().Get(JS_URL_CONSTRUCTOR_NAME).IsUndefined())
        {
            Napi::Function func = DefineClass(
                ...
                JS_URL_CONSTRUCTOR_NAME,
                {
                    ...
                    // Static methods. WebIDL static operations are writable, enumerable and
                    // configurable (a page can replace URL.createObjectURL); napi_default would
                    // make them read-only and let such an assignment fail silently.
